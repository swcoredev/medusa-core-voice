...
// Voice Assistants API Routes (Direct implementation)
...
app.post('/api/voice-assistants', async (req, res) => {
  try {
    ...
    await client.connect();
    const id = uuidv4();
    const assistantStatus = status || 'active';
    const now = new Date();
    ...
    const result = await client.query(
      `INSERT INTO voice_assistants (id, name, language, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [id, name, language, assistantStatus, now, now]
    );
...
async function startServer() {
  console.log('🚀 Запуск обновленного Medusa backend...');
